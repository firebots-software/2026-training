...
public class TankDriveSubsystem {
  ...
  public TankDriveSubsystem() {
    ...
    CurrentLimitsConfigs cl =
        new CurrentLimitsConfigs().withSupplyCurrentLimit(6).withStatorCurrentLimit(6); // etc
  ...
  private void move(String input, float speed) {
    if (input.equals("clockwise")) {
      setLMotorSpeed(
          speed); // conveniently enough the talonfx class has a thing where you dont need a request
      // (it does it for you)
    }
